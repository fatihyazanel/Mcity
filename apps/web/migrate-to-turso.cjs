...
const LOCAL_DB  = path.join(__dirname, "data.db");
...
async function migrate() {
  ...
  const newsRows = Array.from(news.rows);
  let newsCount = 0;
    ...
    try {
      ...
      await remote.execute({
        sql: `INSERT OR IGNORE INTO news
              ...
              VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
        args: [
          s(row.title), s(row.slug), s(row.excerpt), s(row.content_body),
          s(row.featured_image), s(row.source_name), s(row.source_url),
          s(row.published_at), s(row.locale), s(row.category) ?? "news", s(row.created_at)
        ],
      });
      newsCount++;
    } catch(e) { console.warn("  skip news:", e.message?.slice(0,80)); }
  }
  console.log(`✅ ${newsCount}/${newsRows.length} haber aktarıldı\n`);

  // ── 3. Products ──
  console.log("🛍️  Ürünler aktarılıyor...");
  const products = await local.execute("SELECT * FROM products");
  const prodRows = Array.from(products.rows);
  let prodCount = 0;
    ...
    try {
      ...
      await remote.execute({
        sql: `INSERT OR IGNORE INTO products
              ...
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
        args: [
          s(row.product_name), s(row.product_slug), s(row.product_image), s(row.description_short),
          s(row.price_display), n(row.price_amount), s(row.currency) ?? "GBP",
          s(row.affiliate_link), s(row.partner_name), s(row.locale),
          n(row.in_stock), s(row.category) ?? "merchandise", s(row.created_at)
        ],
      });
      prodCount++;
    } catch(e) { console.warn("  skip product:", e.message?.slice(0,80)); }
  }
  console.log(`✅ ${prodCount}/${prodRows.length} ürün aktarıldı\n`);
